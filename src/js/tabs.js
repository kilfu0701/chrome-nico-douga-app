define(['constants', 'storage', 'tab_content', 'util'],
    ...
    function init(templates) {
        ...
        // Add existing tabs
        storage.loadTabs(function(tabs) {
            var keys = Object.keys(tabs.tabs);

            // if there's no tab, add a default tab.
            if (keys.length === 0) {
                openNewRootTab(null, false);
            }
    ...
    $.extend(Tab.prototype, {
        ...
        update_tab_text: function() {
            ...
            this.$node.children('.tab').find('.button .label').text(_text).attr('title', _text);
        },
        ...
        close_tab: function() {
            ...
            // Delete Tab Data
            storage.setTabData(this.id, undefined);
        },
    ...
    return {
        init: init,
        openNewTab: openNewTab,
        openNewRootTab: openNewRootTab,
        focusOnFirstTab: focusOnFirstTab,
